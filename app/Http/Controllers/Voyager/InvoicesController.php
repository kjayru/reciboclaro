...
class InvoicesController extends VoyagerBaseController
{
    ...
    { 
        ...
        $positions = Position::where('invoice_item_id',$id)->get();
        $items = InvoiceItem::where('id',$id)->first();
    ...
    public function setpoints(Request $request)
    {
        ...
        $bheight = Input::get('bheight');
       
        if($request->esmovil){
            $invoice = InvoiceItem::find($request->invoice_item_id);
    ...
    if($latx){
       if(count($latx)>1){
        foreach($latx as $key=>$n){
            ...
            $point = new Position;
            $point->invoice_item_id = $request->invoice_item_id;
            $point->latx = $latx[$key];
            $point->laty = $laty[$key];
    ...
    if($bx){
       if(count($bx)>1){
            ...
            {
                $layer = new Layer;
                $layer->invoice_item_id = $request->invoice_item_id;
                $layer->latx = $bx[$key];
                $layer->laty = $by[$key];
                $layer->width_box = $bwidth[$key];
                $layer->height_box = $bheight[$key];
                ...
                $layer->latx = $bx[0];
                $layer->laty = $by[0];
                $layer->width_box = $bwidth[0];
                $layer->height_box = $bheight[0];
