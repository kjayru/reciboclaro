...
$(document).on('ready', function() {
    ...
    ///22/10/18
    var facturation = Array.apply(null, document.querySelectorAll('.facturation__list ul li h3.links'));
  ...
  facturation.filter(function (element, index) {
    ...
    if (location.hash.length) {
        ...
        if (qtpem.indexOf(location.hash) > -1) {
            ...
            $(location.hash).addClass('act');
            $(location.hash + '-mobile').addClass('act');
            $('.glosario__item a[href="' + location.hash + '"]').parent().addClass('act');
        }
    ...
    $(".cap-qps").click(function(e) {
        e.preventDefault();

        if (block1) {
            if ($(this).parent("article").children("ul").children('li').hasClass('act')) {
    ...
    $(".new-style").click(function(e) {
        e.preventDefault();

        $('.banco__lista.version-mobile-j').toggleClass('active-new')
    });



    $('.quetepasoestemes__item li').on('click', function(event) {
        ...
        if (block1) {
            ...
            if ($(this).hasClass('act') && $(window).outerWidth() <= 700) {
                quetepasoestemesMobile ? ($('.quetepasoestemes__lista').animate({ height: 421 }, 400), quetepasoestemesMobile = false) : ($('.quetepasoestemes__lista').animate({ height: 71 }, 350), quetepasoestemesMobile = true);
                block1 = true;
            } else {
    ...
    $(".dominaturecibo__opciones li a").click(function() {
        var url = $(this).attr('href');

        window.location.href = url;
    });

    /*$(".glosario__lista li a").click(function() {
        var url = $(this).attr('href');
        window.location.href = url;
    });*/
...
function msgs(dia) {
    var pago,
        msg1 = 'Cada ' + dia + ' del mes recibes los megas, minutos y SMS correspondientes a tu plan. Puedes utilizarlos hasta que finalice tu ciclo de facturaciÃ³n',
...
function footer() {
    ...
    puntoFooter>= footerbottom ? $('.banner').addClass('active') : $('.banner').removeClass('active');
}
