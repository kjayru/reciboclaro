...
$(document).on('ready', function(){
	...
	if(location.hash.length){
		var qtpem = ['#interbank','#tengo_una_nueva_linea', '#no_pague_a_tiempo', '#me_cobraron_mas', '#pague_menos', '#me_robaron_mi_celular', '#sali_de_viaje'];
		if(qtpem.indexOf(location.hash)>-1) { 
			$('.listadoStandard, .quetepasoestemes__item, .banco__item').removeClass('act');
...
function msgs(dia){
	var pago, 
		msg1 = 'Cada '+dia+' del mes recibes los megas, minutos y SMS correspondientes a tu plan contratado para utilizarlos hasta que finalice tu ciclo.', 
